...
--comando para confirmar que o path que está sendo usado é o loja
SHOW SEARCH_PATH;
...
COMMENT ON COLUMN Produtos.identidade_dos_produtos IS 'id do produto';
COMMENT ON COLUMN Produtos.nome_produtos IS 'nome do produto';
COMMENT ON COLUMN Produtos.preco_unitario_produtos IS 'preço unitario do produto';
COMMENT ON COLUMN Produtos.detalhes IS 'detalhes do produto';
COMMENT ON COLUMN Produtos.imagem_produtos IS 'imagem do produto';
COMMENT ON COLUMN Produtos.mime_type IS 'formato do arquivo da imagem do produto';
...
COMMENT ON COLUMN Estoques.loja_id_estoques IS 'id da loja';
...
COMMENT ON COLUMN Clientes.id_clientes IS 'id do cliente';
...
COMMENT ON COLUMN Pedidos.id_clientes IS 'id do cliente';
...
COMMENT ON COLUMN Envios.loja_id_envios IS 'id da loja';
COMMENT ON COLUMN Envios.id_clientes_envios IS 'id do cliente';
COMMENT ON COLUMN Envios.endereco_entrega IS 'endereço de entrega';
COMMENT ON COLUMN Envios.status_envio IS 'status do envio';

--criação da tabela pedidos_itens
CREATE TABLE pedidos_itens (
                pedido_id_1 NUMERIC(38) NOT NULL,
                produto_id NUMERIC(38) NOT NULL,
                numero_da_linha NUMERIC(38) NOT NULL,
                preco_unitario NUMERIC(10,2) NOT NULL,
                quantidade_items NUMERIC(38) NOT NULL,
                id_dos_envios NUMERIC(38),
                CONSTRAINT pedido_id_1 PRIMARY KEY (pedido_id_1, produto_id)
);
COMMENT ON TABLE pedidos_itens IS 'Tabela que armazena os itens dos pedidos';
COMMENT ON COLUMN pedidos_itens.pedido_id_1 IS 'id do pedido';
...
COMMENT ON COLUMN pedidos_itens.quantidade_items IS 'quantidade de produtos no pedido';
...
CHECK (status_envio IN ('CRIADO', 'ENVIADO', 'TRANSITO', 'ENTREGUE'));
